...
class Agent():
    def __init__(self):
        self.epsilon = 0
        self.epsilon_step = (config.config.epsilon_start - config.config.epsilon_end) / config.config.epsilon_episode_end
    ...
    def update_epsilon(self, current_episode):
        if current_episode < config.config.epsilon_episode_end:
            self.epsilon = config.config.epsilon_start - self.epsilon_step * current_episode
